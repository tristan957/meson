...
class CoreData:
    ...
    def set_option(self, key: OptionKey, value: T.Union[str, int, bool, T.List[str]]) -> None:
        if key.is_builtin():
            if key.name == 'prefix':
                assert isinstance(value, str), 'for mypy'
...
class BuiltinOption(T.Generic[_T]):
    ...
    def init_option(self, name: 'OptionKey', value: T.Optional[_T], prefix: str) -> UserOption[_T]:
        ...
        # want to do.
        return self.opt_type(self.description, value, self.yielding, **keywords)  # type: ignore
